...
const PayFilter = () => {
    ...
    const [selectedValues, setSelectedValues] = useState(filters.minBasePay);
    const dispatch = useDispatch();

    const extractUniqueCompanies = (data) => {
        const uniqueCompany = new Set(data.map((option) => option.minJdSalary));
        return Array.from(uniqueCompany).sort((a, b) => a - b);
    };
    ...
    const handleChange = (event, newSelectedValues) => {
        ...
        dispatch(updateFilter({ filterType: "minBasePay", value: newSelectedValues.props.value }));
        ...
        <>
            {data && (
                <div className="filter-box">
                    ...
                    >
                        <InputLabel id="demo-simple-select-autowidth-label-1">Minimum Base pay</InputLabel>
                        <Select
                            size="small"
                            fullWidth={true}
